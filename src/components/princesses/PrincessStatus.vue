  ...
  methods: {
    ...
    princessStatus (level, rank, rarity) {
      const stats = level && rank && rarity ? {
        ...
        物理防御力: Math.round(this.rarityStatus(rarity, 'def') + this.promotionStatus(rank, 'def') + this.rarityStatus(rarity, 'def_growth') * (this.level + this.rank)),
        ...
        魔法防御力: Math.round(this.rarityStatus(rarity, 'magic_def') + this.promotionStatus(rank, 'magic_def') + this.rarityStatus(rarity, 'magic_def_growth') * (this.level + this.rank)),
